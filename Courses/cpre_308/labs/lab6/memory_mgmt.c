...
int PRAlgo_FIFO(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
	int frame_to_evict = 0;// To track which frame should be replaced
	int earliest_arrival_time = PageFrames[0].time_of_arrival;//Set time to first frame
    
    // Find the page frame with the earliest arrival time (First-In)
    //Loop through the frames
    for (int i = 1; i < num_frames; i++) {
        if (PageFrames[i].time_of_arrival < earliest_arrival_time) {
            //Check each frame to see if its arrival is earlier than current
            earliest_arrival_time = PageFrames[i].time_of_arrival;//if found then update the earliest time
            frame_to_evict = i;//Update the index of the frame to evict
        }
    }
...
int PRAlgo_LRU(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
    //Track the frame to be replaced
	int frame_to_evict = 0;
    int least_recent_time = PageFrames[0].time_of_access;//Set time to first frame 
    
    // Find the page frame that was least recently used (smallest time_of_access)
    //Loop through the frames from 1
    for (int i = 1; i < num_frames; i++) {
        // check the frames access time is earlier than the current "least recent"
        if (PageFrames[i].time_of_access < least_recent_time) {
            least_recent_time = PageFrames[i].time_of_access;
            frame_to_evict = i; //change frame to evict (index of it)
        }
    }
...
int PRAlgo_OPT(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
	int frame_to_evict = 0;//same as before
    int furthest_access_time = -1;//track the replacement (furthest access so far)
    int page_id;//variable to store page ID
    
    // For each page currently in memory
    for (int i = 0; i < num_frames; i++) {//go through each page in mem
        page_id = PageFrames[i].page_id;//get the page ID
        int next_access_time = -1;// next access time, not found yet
        
        // Find when this page will be accessed next
        for (int j = current_access + 1; j < num_accesses; j++) {//Look to find when the page will be accessed
            //If found we track when it will be accessed
            if (PageAccesses[j] == page_id) {
                next_access_time = j;
                break;
            }
        }
        
        // If this page is never accessed again then it has to be that page 
        if (next_access_time == -1) {
            return i;
        }
        
        // keep track of the page with the furthest next access
        //check and switch to new furthest access we find and update the frame to evict
        if (next_access_time > furthest_access_time) {
